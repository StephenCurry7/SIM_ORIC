...
class ORIC(object):
    ...
    def transform_required_num(self, X, required_num):
        """Return the DataFrame of associated features.
        ...
        column_names = []
        num = 0
        total_num = len(self.rules.items)
        for (antecedents, _), _ in self.rules.items:
            num += 1
            if total_num - num < required_num: #the rule in self.rules is sorted by rconf from low to high, to choose the most number=required_num biggest rules
                feature_name = '__'.join(["{}-{}".format(f, val) for f, val in antecedents])
                if feature_name not in res:
                    idx = [True] * len(X)
                    for f, val in antecedents:
        ...
        return pd.DataFrame(res, index=X.index, columns=column_names, dtype=np.int8)
